...
function buildRecordingsList(nodeId, nodeData) {{
  ...
  // Sort: year asc (nulls last), performances before non-performances,
  // then same-concert tracks by offset_seconds (concert order), then title
  allRecs.sort((a, b) => {{
    if (a.year !== b.year) {{
    ...
    // Same concert (same video_id) → preserve concert order by timestamp
    if (a.vid && a.vid === b.vid) return (a.offset_seconds || 0) - (b.offset_seconds || 0);
